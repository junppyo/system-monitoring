...
SERVER_NAME = server
SERVER_SRC = server.c receiver.c
DB_SRC = saver.c
CLIENT_NAME = client
CLIENT_SRC = sender.c client.c collector.c read_file.c
PRELOAD_NAME = preload.so
PRELOAD_SRC = preload.c utils.c
...
all : $(SERVER_NAME) $(CLIENT_NAME) $(PRELOAD_NAME)

$(PRELOAD_NAME) : $(PRELOAD_SRC)
	$(CC) -fPIC -shared $(PRELOAD_SRC) -o $(PRELOAD_NAME) -ldl
...
fclean :
	rm -rf $(SERVER_OBJECTS) $(CLIENT_OBJECTS) $(UTILS_OBJECTS) $(CLIENT_NAME) $(SERVER_NAME) $(PRELOAD_NAME)
